...
void createTextureImage(vkContext& contextref) {
    ...
    createImage(contextref, texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, 
                ...
                contextref.textureImage, contextref.textureImageMemory);

    transitionImageLayout(contextref, contextref.textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    copyBufferToImage(contextref, stagingBuffer, contextref.textureImage, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
    transitionImageLayout(contextref, contextref.textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
void createTextureSampler(vkContext& contextref) {
    ...
    if (vkCreateSampler(contextref.logicaldevice, &samplerInfo, nullptr, &contextref.textureSampler) != VK_SUCCESS) {
